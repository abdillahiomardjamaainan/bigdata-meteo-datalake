"""
Extraction des données météo depuis OpenWeatherMap
...
DATALAKE_RAW = PROJECT_ROOT / "datalake" / "raw" / "weather"
...
def fetch_weather(latitude, longitude, city_name):
    """
    Appelle l'API Current Weather pour une ville
...
def save_to_raw(data, city_name, run_time):
    ...
    Format: raw/weather/dt=YYYY-MM-DD/run=HHMM/city_name.json
    """
    if data is None:
        return
...
def main():
    ...
    for city in cities:
        ...
        # Appeler l'API
        data = fetch_weather(lat, lon, city_name)
